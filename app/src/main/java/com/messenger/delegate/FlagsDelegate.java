...
public class FlagsDelegate {
   ...
   public Observable<ActionState<GetFlagsAction>> getFlags() {
      Observable<ActionState<GetFlagsAction>> observable = flagsPipe.createObservable(new GetFlagsAction())
            .startWith(getFlagsActionState)
            .filter(actionState -> actionState != null && (actionState.status == ActionState.Status.SUCCESS || actionState.status == ActionState.Status.FAIL))
            .take(1)
            .publish()
            .refCount();
