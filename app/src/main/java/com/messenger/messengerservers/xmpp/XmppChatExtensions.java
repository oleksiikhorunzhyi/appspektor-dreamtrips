...
public class XmppChatExtensions implements ChatExtensions {
   ...
   public Observable<ClearChatEvent> clearChat(String chatId, long clearToDate) {
      return facade.getConnectionObservable()
            .take(1)
            .flatMap(connection -> XmppSendWithResponseObservable.send(connection, new ClearChatIQ(chatId, obtainUserJid(facade
                  .getUsername()), clearToDate)))
   ...
   public Observable<RevertClearingEvent> revertChatClearing(String chatId) {
      return facade.getConnectionObservable()
            .take(1)
            .flatMap(connection -> XmppSendWithResponseObservable.send(connection, new RevertClearChatIQ(chatId, obtainUserJid(facade
                  .getUsername()))))
            .map(stanza -> (RevertClearChatIQ) stanza)
            .map(revertIq -> ImmutableRevertClearingEvent.builder().conversationId(revertIq.getChatId()).build());
   }
}
