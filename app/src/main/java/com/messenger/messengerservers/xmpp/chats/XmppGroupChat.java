...
public class XmppGroupChat extends XmppChat implements GroupChat {
   ...
   protected Observable<MultiUserChat> provideChatObservable() {
      return facade.getConnectionObservable()
            .flatMap(connection -> Observable.fromCallable(() -> createChat(connection)));
   }

   private MultiUserChat createChat(XMPPConnection connection) throws ProtocolException, AccessConversationDeniedException {
   ...
   protected StatusMessageStanza createStatusMessage(String messageId) {
      return new StatusMessageStanza(messageId, Status.DISPLAYED, JidCreatorHelper.obtainGroupJid(roomId), org.jivesoftware.smack.packet.Message.Type.groupchat);
   }
   ...
   public void join(String userId) {
      chatActionObservable(chat -> chat.join(userId)).subscribe(defaultOnNextAction, defaultOnErrorAction);
   }
