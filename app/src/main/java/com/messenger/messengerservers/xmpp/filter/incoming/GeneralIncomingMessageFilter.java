...
public class GeneralIncomingMessageFilter extends BaseIncomingMessageFilter {
   ...
   private boolean isMessageIgnored(int packetType, Message message) {
      ...
      boolean ownMessage = message.getType() == Message.Type.groupchat && JidCreatorHelper.obtainId(message.getTo())
            .equals(JidCreatorHelper.obtainUserIdFromGroupJid(message.getFrom()));
