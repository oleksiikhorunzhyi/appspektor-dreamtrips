...
public abstract class BaseConversationProvider<T extends IQ> extends IQProvider<T> {
   ...
   @NonNull
   private Conversation parseConversation(XmlPullParser parser) throws IOException, XmlPullParserException {
   ...
   public String obtainConversationStatus(long leftTime, boolean kicked) {
      return kicked ? ConversationStatus.KICKED : leftTime > 0 ? ConversationStatus.LEFT : ConversationStatus.PRESENT;
   }

   @Nullable
   private Message parseLastMessage(XmlPullParser parser, String thread, long timestamp) throws IOException, XmlPullParserException {
      ...
      return new Message.Builder().id(messageId)
            .date(timestamp)
            .fromId(from)
