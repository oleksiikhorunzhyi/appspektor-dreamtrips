...
public class NotificationDataFactory {
    ...
    @Inject NotificationDataFactory(ConversationsDAO conversationsDAO, ParticipantsDAO participantsDAO,
                            AttachmentDAO attachmentDAO, UsersDAO usersDAO) {
    ...
    public Observable<NotificationData> createNotificationData(DataMessage message) {
        return  conversationsDAO.getConversation(message.getConversationId()).take(1)
    ...
    private Observable<NotificationData> createNotificationData(DataConversation conversation, DataMessage message) {
        if (ConversationHelper.isSingleChat(conversation)) {
            return Observable.zip(
                            ...
                            (sender, attachment) ->
                                    createChatNotification(sender, message, attachment, conversation));
        } else {
            return Observable.zip(
                            usersDAO.getUserById(message.getFromId()).take(1),
                            attachmentDAO.getAttachmentByMessageId(message.getId()).take(1),
                            participantsDAO.getParticipantsEntities(message.getConversationId()).take(1),
                            (sender, attachment, participants) -> createGroupNotification(
                                    sender, message, attachment, conversation, participants));
        }

    }

    private NotificationData createChatNotification(DataUser sender, DataMessage message, DataAttachment attachment,
    ...
    private NotificationData createGroupNotification(DataUser sender, DataMessage message, DataAttachment attachment,
                                                     DataConversation conversation, List<DataUser> participants) {
    ...
    private boolean filterConversation(DataConversation conversation) {
        return conversation != null && TextUtils
                .equals(conversation.getStatus(), ConversationStatus.PRESENT);
    }
}
