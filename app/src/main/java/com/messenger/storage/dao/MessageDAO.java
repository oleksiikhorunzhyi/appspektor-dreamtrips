...
public class MessageDAO extends BaseDAO {
   ...
   public Observable<Cursor> getMessagesBySyncTime(String conversationId, long syncTime) {
      ...
      return query(q, DataMessage.CONTENT_URI, DataUser.CONTENT_URI, DataTranslation.CONTENT_URI, DataAttachment.CONTENT_URI, DataPhotoAttachment.CONTENT_URI, DataLocationAttachment.CONTENT_URI);
   }
   ...
   public Observable<Integer> markMessagesAsRead(String conversationId, String userId, long visibleTime) {
      return Observable.create(subscriber -> {
         ...
         try {
            subscriber.onNext(getContentResolver().update(DataMessage.CONTENT_URI, cv, clause, new String[]{conversationId, String.valueOf(visibleTime), userId, String.valueOf(MessageStatus.SENT)}));
   ...
   public void deleteMessageByIds(List<String> messageIds) {
      getContentResolver().delete(DataMessage.CONTENT_URI, DataMessage$Table._ID + " IN (?)", new String[]{TextUtils.join(",", messageIds)});
   }
   ...
   public int markSendingAsFailed() {
      ...
      return getContentResolver().update(DataMessage.CONTENT_URI, cv, DataMessage$Table.STATUS + "=?", new String[]{String.valueOf(MessageStatus.SENDING)});
   }
}
