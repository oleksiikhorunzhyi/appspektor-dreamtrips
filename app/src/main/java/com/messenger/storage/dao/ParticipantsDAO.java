...
public class ParticipantsDAO extends BaseDAO {
   ...
   public Observable<List<Pair<DataUser, String>>> getParticipants(String conversationId) {
      ...
      RxContentResolver.Query q = new RxContentResolver.Query.Builder(null).withSelection(participantsSelection("*, " + affiliation))
            .withSelectionArgs(new String[]{conversationId})
            .withSortOrder(userOrder())
            .build();

      return query(q, DataUser.CONTENT_URI, DataParticipant.CONTENT_URI).map(this::convertToListUserWithAffiliation);
   }
   ...
   public Observable<List<DataUser>> getParticipantsEntities(String conversationId) {
      RxContentResolver.Query q = new RxContentResolver.Query.Builder(null).withSelection(participantsSelection("*"))
            .withSelectionArgs(new String[]{conversationId})
            .withSortOrder(userOrder())
            .build();

      return query(q, DataUser.CONTENT_URI, DataParticipant.CONTENT_URI).compose(DaoTransformers.toEntityList(DataUser.class));
   }
