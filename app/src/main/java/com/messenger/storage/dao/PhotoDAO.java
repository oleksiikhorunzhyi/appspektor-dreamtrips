...
public class PhotoDAO extends BaseAttachmentDAO<DataPhotoAttachment> {
   ...
   public Observable<List<DataPhotoAttachment>> getPhotoAttachments(String conversationId) {
      ...
      return query(q, DataPhotoAttachment.CONTENT_URI).compose(DaoTransformers.toEntityList(DataPhotoAttachment.class));
   }
   ...
   public Observable<List<DataPhotoAttachment>> getErrorAttachments() {
      ...
      return query(q, DataPhotoAttachment.CONTENT_URI, DataMessage.CONTENT_URI).compose(DaoTransformers.toEntityList(DataPhotoAttachment.class));
   }
   ...
   protected <T extends BaseModel> void bulkInsert(List<T> collection, ModelAdapter<T> adapter, Uri uri) {
      ...
      for (T t : collection) {
         ...
         int updated = db.update(adapter.getTableName(), values, DataPhotoAttachment$Table.PHOTOATTACHMENTID + "=?", new String[]{attachment.getPhotoAttachmentId()});
         if (updated == 0) {
            db.insertWithOnConflict(adapter.getTableName(), null, values, ConflictAction.getSQLiteDatabaseAlgorithmInt(adapter
   ...
   public int markSendingAsFailed() {
      ...
      return getContentResolver().update(DataPhotoAttachment.CONTENT_URI, cv, DataPhotoAttachment$Table.UPLOADSTATE + "=?", new String[]{String.valueOf(PhotoAttachmentStatus.UPLOADING)});
   }
}
