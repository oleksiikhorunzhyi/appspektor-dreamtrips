...
public class UsersDAO extends BaseDAO {
   ...
   public Observable<DataUser> getUserById(String id) {
      RxContentResolver.Query q = new RxContentResolver.Query.Builder(null).withSelection("SELECT * FROM " + DataUser.TABLE_NAME + " WHERE " + DataUser$Table._ID + "=?")
            .withSelectionArgs(new String[]{String.valueOf(id)})
            .build();
      return query(q, DataUser.CONTENT_URI).compose(DaoTransformers.toEntity(DataUser.class));
   }
   ...
   public Observable<List<DataUser>> getFriends(String currentUserId) {
      RxContentResolver.Query q = new RxContentResolver.Query.Builder(null).withSelection("SELECT * FROM " + DataUser.TABLE_NAME + " " +
            ...
            .build();
      return query(q, DataUser.CONTENT_URI).compose(DaoTransformers.toEntityList(DataUser.class));
   }
