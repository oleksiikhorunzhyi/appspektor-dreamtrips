...
public class ConversationLastMessageInflater extends ViewInflater {
   ...
   private String createMessageText(DataConversation dataConversation, DataMessage message, DataUser messageAuthor, DataUser recipient, DataUser currentUser, String attachmentType, DataTranslation dataTranslation) {
      ...
      if (message.getType() != null) {
         if (MessageHelper.isUserMessage(message)) {
            ...
            messageText = systemMessageTextProvider.getSystemMessageText(dataConversation.getType(), message, messageAuthor, recipient)
                  .toString();
         }
   ...
   private String createImageAttachmentText(DataMessage message, DataUser messageAuthor, DataUser currentUser) {
      if (TextUtils.equals(message.getFromId(), currentUser.getId())) {
         ...
         return messageAuthor.getDisplayedName()
               .trim() + " " + context.getString(R.string.conversation_list_item_last_message_image);
      }
   }
   ...
   private String createLocationAttachmentText(DataMessage message, DataUser messageAuthor, DataUser currentUser) {
      if (TextUtils.equals(message.getFromId(), currentUser.getId())) {
         ...
         return messageAuthor.getDisplayedName()
               .trim() + " " + context.getString(R.string.conversation_list_item_last_message_location);
      }
   }
}
