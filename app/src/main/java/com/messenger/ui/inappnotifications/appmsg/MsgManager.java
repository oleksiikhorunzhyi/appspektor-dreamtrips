...
import static com.messenger.ui.inappnotifications.appmsg.AppMsg.LENGTH_STICKY;
...
class MsgManager extends Handler implements Comparator<AppMsg> {
   ...
   void add(AppMsg appMsg) {
      msgQueue.add(appMsg);
      if (appMsg.inAnimation == null) {
         appMsg.inAnimation = AnimationUtils.loadAnimation(appMsg.getActivity(), android.R.anim.fade_in);
      }
      if (appMsg.outAnimation == null) {
         appMsg.outAnimation = AnimationUtils.loadAnimation(appMsg.getActivity(), android.R.anim.fade_out);
      }
      displayMsg();
   }
   ...
   private void displayMsg() {
      ...
      if (!appMsg.isShowing()) {
         ...
         sendMessageDelayed(msg, appMsg.getDuration() + appMsg.inAnimation.getDuration() + appMsg.outAnimation.getDuration());
      }
   }
   ...
   private void removeMsg(final AppMsg appMsg) {
      ...
      if (parent != null) {
         appMsg.outAnimation.setAnimationListener(new OutAnimationListener(appMsg));
         view.clearAnimation();
         view.startAnimation(appMsg.outAnimation);
      }
   ...
   private void addMsgToView(AppMsg appMsg) {
      ...
      view.clearAnimation();
      view.startAnimation(appMsg.inAnimation);
   ...
   public int compare(AppMsg lhs, AppMsg rhs) {
      return inverseCompareInt(lhs.priority, rhs.priority);
   }
