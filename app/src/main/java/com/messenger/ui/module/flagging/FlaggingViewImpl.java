...
public class FlaggingViewImpl extends ModuleStatefulViewImpl<FlaggingPresenter>
        implements FlaggingView {
    ...
    public Observable<CharSequence> showFlagReasonDialog(Flag flag, String prefilledReason) {
        ...
        reasonObservable.subscribe(text
                -> positiveButton.setEnabled(text.toString().trim().length() > 0));
        activeDialog = dialog;
    ...
    private void showProgressDialog(@StringRes int message) {
        activeDialog = ProgressDialog.show(getContext(), "",
