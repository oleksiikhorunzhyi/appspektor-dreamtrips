...
public abstract class ChatMembersScreenPresenterImpl extends MessengerPresenterImpl<ChatMembersScreen, ChatMembersScreenViewState> implements ChatMembersScreenPresenter {
   ...
   @Nullable private String lastSearchQuery;
   ...
   protected Observable<List<DataUser>> createContactListObservable() {
      return usersDAO.getFriends(user.getId()).compose(bindView()).subscribeOn(Schedulers.io());
   }

   protected void connectToContacts() {
      getExistingAndFutureParticipants().compose(bindView())
            .flatMap(existingAndFutureParticipantsPair -> searchHelper.search(createContactListObservable(), getView().getSearchQueryObservable()
                  .doOnNext(query -> lastSearchQuery = query.toString()), (user, filter) -> StringUtils.containsIgnoreCase(user
                  .getDisplayedName(), filter))
                  .compose(userSectionHelper.prepareItemInCheckableList(existingAndFutureParticipantsPair.first, existingAndFutureParticipantsPair.second))
                  .map(pair -> pair.first))
   ...
   public void applyViewState() {
      ...
      getChatNameShouldBeVisibleObservable().subscribe(visible -> getView().setConversationNameEditTextVisibility(visible ? View.VISIBLE : View.GONE));
   ...
   private void bindToRemovedUserObservable() {
      getView().getRemovedUserObservable().compose(bindView()).subscribe(this::handleRemovedUser);
   }
