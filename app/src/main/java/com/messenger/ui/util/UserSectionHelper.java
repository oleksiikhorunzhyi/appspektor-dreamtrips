...
public class UserSectionHelper {
   ...
   public Observable.Transformer<List<DataUser>, Pair<List<Object>, Integer>> prepareItemInCheckableList(Collection<DataUser> participants, Collection<DataUser> selectedUsers) {
      return listObservable -> listObservable.map(dataUsers -> {
         ...
         return res;
      }).map(userItems -> new Pair<>(prepareContactGroups(userItems), userItems.size())).map(this::mapToItem);
   }
   ...
   public Observable.Transformer<List<Pair<DataUser, String>>, Pair<List<Object>, Integer>> groupTransformer(Observable<DataConversation> collectionObservable) {
      return pairObservable -> pairObservable.withLatestFrom(collectionObservable, (pairs, conversation) -> new Pair<>(prepareMemberGroups(pairs, conversation), pairs
            .size())).map(this::mapToItem);
   }
   ...
   private List<Group<SwipeDataUser>> prepareMemberGroups(List<Pair<DataUser, String>> members, DataConversation conversation) {
      ...
      Map<String, Collection<SwipeDataUser>> groupedMap = Queryable.from(members)
            .groupToMap(pair -> getUserGroup(pair.first, pair.second, isTripConversation), pair -> toSwipeDataUser(pair.first, pair.second, isAdmin, enableOnlineMembersStatuses));
   ...
   private SwipeDataUser toSwipeDataUser(DataUser user, String affiliation, boolean isOwner, boolean onlineStatusEnabled) {
      return new SwipeDataUser(user, isOwner && !TextUtils.equals(affiliation, Affiliation.OWNER), onlineStatusEnabled);
   }
   ...
   private String getUserGroup(DataUser user, String affiliation, boolean isTripConversation) {
      if (TextUtils.equals(affiliation, Affiliation.OWNER)) {
         return ADMIN_TYPE;
      } else if (isTripConversation && user.isHost()) { return HOST_TYPE; } else {
