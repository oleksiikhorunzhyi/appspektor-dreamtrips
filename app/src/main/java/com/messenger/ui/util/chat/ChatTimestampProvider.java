...
public class ChatTimestampProvider {
   ...
   public String getTimestamp(Cursor cursor) {
      return timestampFormatter.getMessageTimestamp(cursor.getLong(cursor.getColumnIndex(DataMessage$Table.DATE)));
   }
   ...
   public boolean shouldShowAutomaticTimestamp(Cursor cursor) {
      ...
      if (previousDate != 0) {
         calendarDaysSincePreviousDate = (int) ChatDateUtils.calendarDaysBetweenDates(previousDate, currentDate);
      }
