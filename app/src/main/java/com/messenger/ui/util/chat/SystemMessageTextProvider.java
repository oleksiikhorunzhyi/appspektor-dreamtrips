...
public class SystemMessageTextProvider {
   ...
   private Spanned obtainJoinSystemMessage(String conversationType, DataUser sender, DataUser recipient) {
      if (ConversationHelper.isTripChat(conversationType)) {
         ...
         return Html.fromHtml(resources.getString(R.string.system_message_added, obtainAdminText(sender), obtainUserTextWithoutCapitalLetter(recipient)));
      }
   }
   ...
   private Spanned obtainKickSystemMessage(String conversationType, DataUser sender, DataUser recipient) {
      if (ConversationHelper.isTripChat(conversationType)) {
         ...
         return Html.fromHtml(resources.getString(R.string.system_message_removed, obtainAdminText(sender), obtainUserTextWithoutCapitalLetter(recipient)));
      }
   }

   private Spanned obtainLeftSystemMessage(DataUser sender) {
      return Html.fromHtml(isItMe(sender) ? resources.getString(R.string.system_message_you_left_the_chat) : resources.getString(R.string.system_message_left, sender
            .getDisplayedName()));
   }
