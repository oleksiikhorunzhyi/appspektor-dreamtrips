...
public abstract class InAppMessengerNotificationView extends BaseInAppNotificationView {
   @InjectView(R.id.in_app_notif_title) TextView tvTitle;
   @InjectView(R.id.in_app_notif_text) TextView tvText;
   ...
   protected String getMessageText(DataUser sender, DataMessage dataMessage, @Nullable String attachmentType) {
      return MessageVersionHelper.isUnsupported(attachmentType) ? Html.fromHtml(getResources().getString(R.string.chat_update_proposition))
            .toString() : dataMessage.getText();
   }
}
