...
@CommandAction
public class RecentlyAddedBucketsFromPopularCommand extends Command<Pair<BucketType, List<BucketItem>>>
        implements CachedAction<List<BucketItem>> {
    ...
    public static RecentlyAddedBucketsFromPopularCommand add(BucketItem item) {
        return new RecentlyAddedBucketsFromPopularCommand(new AddFunc(item), BucketType.valueOf(item.getType().toUpperCase()));
    }
    ...
    private RecentlyAddedBucketsFromPopularCommand(Func1<List<BucketItem>, List<BucketItem>> func,
