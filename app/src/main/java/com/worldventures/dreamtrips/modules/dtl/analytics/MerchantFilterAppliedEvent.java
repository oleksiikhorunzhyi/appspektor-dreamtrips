...
public class MerchantFilterAppliedEvent extends DtlAnalyticsAction {
   ...
   @Attribute("dtlprice") final String price;

   @Attribute("dtldistance") final String distance;

   @Attribute("amenities") final String amenities;

   public MerchantFilterAppliedEvent(DtlFilterData filterData) {
      price = String.format(Locale.US, "%d-%d", filterData.getMinPrice(), filterData.getMaxPrice());
      distance = String.format(Locale.US, "10-%.0f%s", filterData.getMaxDistance(), filterData.getDistanceType()
            .getTypeNameForAnalytics());
   ...
   private static String joinAmenities(List<DtlMerchantAttribute> amenities) {
      ...
      return Queryable.from(amenities).map(DtlMerchantAttribute::getName).joinStrings(",");
   }
}
