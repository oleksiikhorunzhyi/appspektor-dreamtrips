...
import com.worldventures.dreamtrips.modules.dtl.helper.DtlMerchantManyImagesDataInflater;
...
public class DtlMerchantDetailsFragment
        extends RxBaseFragmentWithArgs<DtlMerchantDetailsPresenter, MerchantDetailsBundle>
    ...
    protected DtlMerchantDetailsPresenter createPresenter(Bundle savedInstanceState) {
        return new DtlMerchantDetailsPresenter(getArgs().getDtlMerchant());
    }
    ...
    private void setAdditional(DtlMerchant merchant) {
        Queryable.from(helper.getContactsData(merchant)).forEachR(contact -> {
            TextView contactView = (TextView) LayoutInflater.from(getActivity()).inflate(R.layout.list_item_dtl_merchant_contact, additionalContainer, false);
    ...
    @Override
    public void openSuggestMerchant(SuggestMerchantBundle data) {
