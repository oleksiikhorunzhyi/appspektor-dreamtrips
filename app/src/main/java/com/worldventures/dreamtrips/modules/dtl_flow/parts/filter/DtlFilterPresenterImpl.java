...
public class DtlFilterPresenterImpl implements DtlFilterPresenter {
   ...
   private void bindFilterUpdates() {
      filterInteractor.filterDataPipe().observeSuccess().map(DtlFilterDataAction::getResult).observeOn(AndroidSchedulers
            .mainThread()).takeUntil(detachStopper.asObservable()).subscribe(view::syncUi);
   }
