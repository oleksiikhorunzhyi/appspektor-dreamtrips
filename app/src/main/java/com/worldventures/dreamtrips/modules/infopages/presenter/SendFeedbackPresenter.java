...
public class SendFeedbackPresenter extends Presenter<SendFeedbackPresenter.View> {
   ...
   public void sendFeedback(int feedbackType, String text) {
      List<FeedbackImageAttachment> attachments =
            Queryable.from(attachmentsManager.getAttachments()).map(EntityStateHolder::entity).toList();

      view.changeDoneButtonState(false);

      feedbackInteractor.getSendFeedbackPipe()
            .createObservable(new SendFeedbackCommand(feedbackType, text, attachments))
   ...
   public void onShowMediaPicker() {
      view.showMediaPicker(PICKER_REQUEST_ID, PICKER_MAX_IMAGES);
   }
   ...
   private void removeOldAttachment(EntityStateHolder<FeedbackImageAttachment> holder) {
