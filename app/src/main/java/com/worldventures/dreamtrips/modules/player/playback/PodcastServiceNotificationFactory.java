...
public class PodcastServiceNotificationFactory {
   ...
   private NotificationCompat.Builder getDefaultNotification() {
      NotificationCompat.Builder notification = new NotificationCompat.Builder(context).setSmallIcon(R.drawable.dt_push_icon)
   ...
   private NotificationCompat.Action getPauseAction() {
      ...
      return new NotificationCompat.Action.Builder(R.drawable.notification_player_icon_pause, res.getString(R.string.podcasts_button_pause), PendingIntent
            .getService(context, 0, intent, 0)).build();
   }

   private NotificationCompat.Action getPlayAction() {
      Intent intent = new Intent(context, PodcastService.class);
      intent.setAction(PodcastService.NOTIFICATION_ACTION_PLAY);
      return new NotificationCompat.Action.Builder(R.drawable.notification_player_icon_play, res.getString(R.string.podcasts_button_resume), PendingIntent
            .getService(context, 0, intent, 0)).build();
   }
   ...
   private NotificationCompat.Action getStopAction() {
      ...
      return new NotificationCompat.Action.Builder(R.drawable.notification_player_icon_stop, res.getString(R.string.podcasts_button_stop), PendingIntent
            .getService(context, 0, intent, 0)).build();
   }
}
