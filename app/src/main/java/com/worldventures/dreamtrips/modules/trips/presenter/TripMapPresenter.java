...
public class TripMapPresenter extends Presenter<TripMapPresenter.View> {
   ...
   public void onMarkerClicked(Marker marker) {
      MapObjectHolder holder = Queryable.from(mapObjects).firstOrDefault(object -> object.getItem()
            .getCoordinates()
            .getLat() == marker.getPosition().latitude && object.getItem()
            .getCoordinates()
            .getLng() == marker.getPosition().longitude);
   ...
   public void reloadMapObjects() {
      ...
      if (view != null) {
         compositeSubscription.add(tripMapInteractor.mapObjectsPipe()
               .createObservableResult(new GetMapObjectsHttpAction(tripFilterDataProvider.get(), view.getMap()
                     .getProjection()
                     .getVisibleRegion().latLngBounds, query))
               ...
               .observeOn(AndroidSchedulers.mainThread())
               .subscribe(this::onTripMapObjectsLoaded, error -> {
