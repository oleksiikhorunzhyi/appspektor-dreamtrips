...
public class TripViewInjector {
    ...
    public void initTripData(TripModel tripModel, User currentUser) {
        ...
        if (textViewDate != null) {
            textViewDate.setText(tripModel.isHasMultipleDates()
                    ? String.format(textViewDate.getResources().getString(R.string.multiple_dates), tripModel.getAvailabilityDates().getStartDateString())
                    : tripModel.getAvailabilityDates().toString());
        }
    }
}
