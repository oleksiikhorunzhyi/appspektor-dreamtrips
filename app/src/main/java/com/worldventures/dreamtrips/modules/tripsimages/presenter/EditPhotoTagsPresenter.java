...
public class EditPhotoTagsPresenter extends Presenter<EditPhotoTagsPresenter.View>
        implements PhotoTagHolderManager.FriendRequestProxy {
    ...
    public void requestFriends(String query, int page, Action1<List<User>> act) {
        doRequest(new GetFriendsQuery(null, query, page, PAGE_SIZE),
                friends -> act.call(Queryable.from(friends).filter(user -> !isUserExists(user)).toList()));
    }
