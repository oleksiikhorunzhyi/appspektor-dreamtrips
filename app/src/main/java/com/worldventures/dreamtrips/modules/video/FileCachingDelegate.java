...
public class FileCachingDelegate {
   ...
   public FileCachingDelegate(SnappyRepository db, Context context, Injector injector, FileDownloadSpiceManager spiceManager) {
      this.db = db;
   ...
   protected void startCaching(CachedEntity entity, String path) {
      ...
      fileDownloadSpiceManager.cancel(InputStream.class, entity.getUuid());
      fileDownloadSpiceManager.execute(bigBinaryRequest, entity.getUuid(), DurationInMillis.ALWAYS_RETURNED, requestListener);
      view.notifyItemChanged(entity);
   }
