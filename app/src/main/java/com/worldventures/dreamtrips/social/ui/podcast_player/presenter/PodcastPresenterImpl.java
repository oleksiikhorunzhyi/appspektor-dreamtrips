...
import com.worldventures.dreamtrips.social.ui.podcast_player.playback.ReadOnlyPlayer;
...
public class PodcastPresenterImpl extends DtlPresenterImpl<PodcastPlayerScreen, ViewState.EMPTY> implements PodcastPresenter {
   ...
   public void attachView(PodcastPlayerScreen view) {
      ...
      playerObservable = podcastPlayerDelegate.createPlayer(uri).replay(1).autoConnect();
      startPlayback();
   ...
   private void startPlayback() {
      playerObservable.compose(bindView()).take(1).subscribe(player -> {
         if (player.getState() == ReadOnlyPlayer.State.READY) {
            podcastPlayerDelegate.start();
         } else if (player.getState() == ReadOnlyPlayer.State.STOPPED || player.getState() == ReadOnlyPlayer.State.RELEASED) {
   ...
   private void listenToStateUpdates() {
      playerObservable.flatMap(ReadOnlyPlayer::getStateObservable)
   ...
   private void listenToProgress() {
      Observable.interval(0, 1, TimeUnit.SECONDS, AndroidSchedulers.mainThread())
            ...
            .compose(bindView())
            .filter(player -> player.getState() != ReadOnlyPlayer.State.UNKNOWN && player.getState() != ReadOnlyPlayer.State.PREPARING)
            .subscribe(player -> {
   ...
   private void updatePlayerProgress(ReadOnlyPlayer player) {
      getView().setProgress(player.getDuration(), player.getCurrentPosition(), player.getBufferPercentage());
   }
   ...
   public void playPause() {
      podcastPlayerDelegate.createPlayer(uri).subscribe(player -> {
