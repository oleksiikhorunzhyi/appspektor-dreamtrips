...
public class DtlFilterMerchantInteractorTest extends DtlBaseMerchantServiceTest {
   ...
   public void testFilterInit() {
      checkFilterAction(DtlFilterDataAction.init(), action -> action.getResult().isDefault() && action.getResult()
            .getDistanceType() == DistanceType.provideFromSetting(testDistanceSetting));
   }
   ...
   public void testFilterAmenitiesUpdate() {
      ...
      checkFilterAction(DtlFilterDataAction.amenitiesUpdate(Collections.singletonList(amenity)), action -> action.getResult()
            .hasAmenities() && action.getResult().getAmenities().contains(amenity));
   }
   ...
   public void testFilterApplyParams() {
      ...
      checkFilterAction(DtlFilterDataAction.applyParams(parameters), action -> !action.getResult().isDefault() && action
            .getResult()
   ...
   public void testFilterApplySearch() {
      String query = "test";
      checkFilterAction(DtlFilterDataAction.applySearch(query), action -> action.getResult()
            .getSearchQuery()
            .equals(query));
   }
   ...
   public void testFilterApplyOffersOnly() {
      checkFilterAction(DtlFilterDataAction.applyOffersOnly(true), action -> action.getResult().isOffersOnly());
   ...
   public void testDtlFilterMerchantsAction() {
      ...
      locationInteractor.locationPipe().send(DtlLocationCommand.change(ImmutableDtlManualLocation.builder()
   ...
   private void checkFilterAction(DtlFilterDataAction filterDataAction, Func1<DtlFilterDataAction, Boolean> assertPredicate) {
      ...
      filterMerchantInteractor.filterDataPipe().createObservable(filterDataAction).subscribe(subscriber);
