...
public class DtlLocationInteractorTest extends BaseTest {
   ...
   static {
      testLocationList = Collections.singletonList(testLocation = mock(DtlExternalLocation.class));
   ...
   public void setup() throws NoSuchFieldException {
      ...
      Janet janet = new Janet.Builder().addService(cachedService(daggerActionService = new MockDaggerActionService(new CommandActionService())))
            .addService(cachedService(httpStubWrapper = new StubServiceWrapper(new MockHttpActionService.Builder().bind(new MockHttpActionService.Response(200)
                  .body(testLocationList), request -> request.getUrl().contains("/locations")).build())))
            .build();
   ...
   public void testDtlLocationCommand() {
      ...
      locationInteractor.locationPipe().createObservable(DtlLocationCommand.last()).subscribe(subscriber);
      assertActionSuccess(subscriber, action -> action.getResult()
            .getLocationSourceType() == LocationSourceType.UNDEFINED);

      subscriber = new TestSubscriber<>();
      locationInteractor.locationPipe().createObservable(DtlLocationCommand.change(testLocation)).subscribe(subscriber);
   ...
   public void testDtlSearchLocationAction() {
      ...
      StubServiceWrapper.Callback spyHttpCallback = spy(StubServiceWrapper.Callback.class);
      httpStubWrapper.setCallback(spyHttpCallback);
      locationInteractor.searchLocationPipe().createObservable(new DtlSearchLocationAction(testLocation.getLongName()
            .substring(0, 2))).subscribe(subscriber);
      assertActionSuccess(subscriber, action -> action.getResult().isEmpty());
      ...
      httpStubWrapper.setCallback(spyHttpCallback);
      locationInteractor.searchLocationPipe().createObservable(new DtlSearchLocationAction(testLocation.getLongName()
            .substring(0, 3))).subscribe(subscriber);
      assertActionSuccess(subscriber, action -> !action.getResult().isEmpty());
      verify(spyHttpCallback, times(1)).onSend(any(ActionHolder.class));
