...
public class DtlTransactionInteractorTest extends BaseTest {
   ...
   public void setup() throws Exception {
      ...
      Janet janet = new Janet.Builder().addService(cachedService(daggerActionService = new MockDaggerActionService(new CommandActionService())))
            .addService(cachedService(new MockHttpActionService.Builder().bind(new MockHttpActionService.Response(200).body(testEstimationResult), request -> request
                  .getUrl()
                  .contains("/estimations"))
                  .bind(new MockHttpActionService.Response(200).body(testTransactionResult), request -> request.getUrl()
                        .contains("/transactions"))
                  .bind(new MockHttpActionService.Response(200), request -> request.getUrl().contains("/ratings"))
                  .build()))
            .build();
   ...
   public void testTransactionGet() {
      checkTransactionAction(DtlTransactionAction.get(testMerchant), action -> action.getResult()
   ...
   public void testTransactionUpdate() {
      ...
      checkTransactionAction(DtlTransactionAction.update(testMerchant, transaction -> ImmutableDtlTransaction.copyOf(transaction)
            .withMerchantToken(merchantToken)), action -> action.getResult().getMerchantToken().equals(merchantToken));
      verify(db, times(1)).getDtlTransaction(anyString());
   ...
   public void testTransactionSave() {
      DtlTransaction transaction = ImmutableDtlTransaction.copyOf(testTransaction).withMerchantToken("test");
      checkTransactionAction(DtlTransactionAction.save(testMerchant, transaction), action -> action.getResult()
            .equals(transaction));
      verify(db, times(1)).getDtlTransaction(anyString());
   ...
   public void testTransactionClean() {
      checkTransactionAction(DtlTransactionAction.clean(testMerchant), action -> action.getResult()
            .getMerchantToken() == null);
      verify(db, times(1)).getDtlTransaction(anyString());
      verify(db, times(1)).saveDtlTransaction(anyString(), any(DtlTransaction.class));
   }
   ...
   public void testDtlEstimatePointsAction() {
      ...
      transactionInteractor.estimatePointsActionPipe()
            .createObservable(new EstimationHttpAction(testMerchant.getId(), ImmutableEstimationParams.builder()
   ...
   public void testRatingHttpAction() {
      ...
      transactionInteractor.rateActionPipe()
            .createObservable(new RatingHttpAction(testMerchant.getId(), ImmutableRatingParams.builder()
                  .rating(5)
                  .transactionId(testTransaction.getDtlTransactionResult().getId())
                  .build()))
            .subscribe(subscriber);
