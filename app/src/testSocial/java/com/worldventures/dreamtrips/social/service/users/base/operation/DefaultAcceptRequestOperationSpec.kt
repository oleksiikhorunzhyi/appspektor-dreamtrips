...
class DefaultAcceptRequestOperationSpec : BaseBodySpec(object : BaseUserStorageOperationTestBody() {
   override fun create(): Spec.() -> Unit = {
      describe("Perform operation") {
         it("Should remove user from list and shouldn't change canLoadMoreValue") {
            ...
            DefaultAcceptRequestOperation(user).perform(users, { canLoadMore = it })
