...
import com.worldventures.core.utils.SdkVersionUtils;
...
public class PermissionDispatcher {
   ...
   private Observable<PermissionsResult> doesNotHaveSelfPermissions(Activity activity, String[] permissions, int requestCode, boolean shouldShowRequestRationale) {
      if (shouldShowRequestRationale && shouldShowRequestPermissionRationale(activity, permissions)) {
         ...
         return permissionResultBus.filter(result -> result.requestCode == requestCode).take(1);
      }
   }
