...
public class PinProgressButton extends CompoundButton {
   ...
   private void init(Context context, AttributeSet attrs, int defStyle) {
      ...
      int progressColor = res.getColor(R.color.pin_progress_color_progress);
      int errorColor = res.getColor(R.color.pin_progress_color_error);
      ...
      progressDrawable = res.getDrawable(R.drawable.ic_pin_btn_progress_drawable);
      progressDrawable.setCallback(this);
      errorDrawable = res.getDrawable(R.drawable.ic_pin_btn_error_drawable);
      errorDrawable.setCallback(this);
      successDrawable = res.getDrawable(R.drawable.ic_pin_btn_success_drawable);
      successDrawable.setCallback(this);
      shadowDrawable = res.getDrawable(R.drawable.pin_progress_shadow);
      shadowDrawable.setCallback(this);

      mDrawableSize = shadowDrawable.getIntrinsicWidth();
   ...
   protected void drawableStateChanged() {
      ...
      if (successDrawable.isStateful()) {
         successDrawable.setState(getDrawableState());
      }
      if (errorDrawable.isStateful()) {
         errorDrawable.setState(getDrawableState());
      }
   ...
   protected void onDraw(Canvas canvas) {
      ...
      canvas.drawArc(mTempRectF, 0, 360, true, failed ? mErrorPaint : mCirclePaint);
      canvas.drawArc(mTempRectF, -90, 360 * mProgress / mMax, true, mProgressPaint);

      shadowDrawable.setBounds(mTempRect);
      shadowDrawable.draw(canvas);

      mTempRect.set(mDrawableSize / 4, mDrawableSize / 4, mDrawableSize * 3 / 4, mDrawableSize * 3 / 4);
      Drawable iconDrawable = failed ? errorDrawable : mProgress == mMax ? successDrawable : progressDrawable;
