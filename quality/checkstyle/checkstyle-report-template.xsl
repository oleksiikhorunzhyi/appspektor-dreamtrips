<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
   ...
   <xsl:template match="checkstyle" mode="filelist">
      ...
      <table border="0" cellpadding="5" cellspacing="2" class="log" width="100%">
         ...
         <xsl:for-each select="file[@name and generate-id(.) = generate-id(key('files', @name))]">
            ...
            <!-- Sort method 1: Primary by #error, secondary by #warning, tertiary by #javadoc -->
            <xsl:sort data-type="number" order="descending"
                  select="count(key('files', @name)/error[@severity='error'])" />
            <xsl:sort data-type="number" order="descending"
                  select="count(key('files', @name)/error[@severity='warning'])" />
            <xsl:sort data-type="number" order="descending"
                  select="count(key('files', @name)/error[contains(translate(@source, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'javadoc')])" />
            <xsl:sort data-type="number" order="descending"
            ...
            <xsl:variable name="javadocCount"
                  select="count(key('files', @name)/error[contains(translate(@source, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'javadoc')])" />
            ...
            <tr>
               ...
               <td>
                  <xsl:value-of select="$javadocCount" />
               </td>
               <td>
   ...
   <xsl:template match="checkstyle" mode="summary">
      ...
      <!-- Xpath 1.0 :( -->
      <xsl:variable name="javadocCount"
            select="count(file/error[contains(translate(@source, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'javadoc')])" />
      <table border="0" cellpadding="5" cellspacing="2" class="log" width="100%">
         <tr>
            <th>Files</th>
            <th>Errors</th>
