...
public class SmartCardErrorViewProvider<T> implements ErrorViewProvider<T> {
   ...
   static class ErrorView<M> extends DialogErrorView<M> {
      ...
      protected MaterialDialog createDialog(M t, Throwable throwable, Context context) {
         ...
         if (retryAction != null) {
            builder.positiveText(R.string.wallet_retry_label).onPositive((dialog, which) -> retryAction.call(t));
         }
         if (cancelAction != null) {
            builder.negativeText(R.string.wallet_cancel_label).onNegative((dialog, which) -> cancelAction.call(t));
         } else {
            builder.negativeText(R.string.wallet_ok);
         }
