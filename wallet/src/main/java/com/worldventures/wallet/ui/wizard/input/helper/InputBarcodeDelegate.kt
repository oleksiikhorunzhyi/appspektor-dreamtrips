...
class InputBarcodeDelegate(
   ...
   private fun handleSmartCardStatus(inputDelegateView: InputDelegateView, command: GetSmartCardStatusCommand) {
      when (command.result) {
         SmartCardStatus.ASSIGNED_TO_CURRENT_DEVICE -> cardIsUnassigned(command.smartCardId)
         SmartCardStatus.UNASSIGNED -> cardAssignToAnotherDevice(command.smartCardId)
         SmartCardStatus.ASSIGNED_TO_ANOTHER_DEVICE -> inputDelegateView.showErrorCardIsAssignedDialog()
         SmartCardStatus.ASSIGNED_TO_ANOTHER_USER -> fetchAssociatedSmartCard()
